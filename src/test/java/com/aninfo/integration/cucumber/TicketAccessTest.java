...
public class TicketAccessTest extends TicketIntegrationServiceTest {
    ...
    @Then("^I get an Invalid Ticket Exception because the ticket does not exist$")
    public void obtains_invalid_ticket_exception_nonexistent() {
        assertNull(this.ticket);
        assertNotNull(this.invalid_error);
        assertEquals("Ticket not found: MyTicket2", this.invalid_error.getMessage());
    }
